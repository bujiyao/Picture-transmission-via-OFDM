...
Rx_gain = 40;            % 設定rx的功率大小
Tx_gain = 0;           % 設定feedback_tx的功率大小
...
Inx_carrier_FS = 1:fft_size;
Inx_carrier_FS(Inx_carrier) = [];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%   頻率同步用的訊號   %%%%%%%%%%%%%%%%%%%
% RS_FS = CSIRS(0,0,length(Inx_carrier_FS));
% for k = 1:size(tx_data_freq,2)-1
%     tx_data_freq(Inx_carrier_FS,k+1) = RS_FS;
% end
[CellID1,PSS_FS,SSS_FS] = SS(1,0);
PSS_FS = [PSS_FS 0];
for k = 1:size(tx_data_freq,2)-1
    tx_data_freq(Inx_carrier_FS,k+1) = PSS_FS;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%   data   %%%%%%%%%%%%%%%%%%%%%%%%%
...
while true
    ...
    %     interleaver = randperm(info_len);
        ...
        while (CRC_result ==  0)
            close all;
            [tx_radio]=set_pluto_TX(fc,fs,tx_data_seq,Tx_gain);
            ...
            while true
                ...
                [temp, lags] = xcorr(rx_data_seq,sync_time); %交相關
                
    %             figure('Name','時間同步:corr抓時間','NumberTitle','off');
    %             subplot(2,1,1);
    %             plot(lags,abs(temp));grid on; %tx rx交相關圖
                
                %利用最高值，找出每個符合的時間
                XX = max(abs(temp))*0.5;
                [TF2,P2] = islocalmax(abs(temp),'MinProminence',XX);
        ...
        %         stem(lags(TF2),abs(a(TF2))); %符合時間點圖
            ...
            %釋放tx rx接口
            release(rx_radio);release(tx_radio);
            ...
            %確認是否成功同步，其td為0
            [temp , lags] = xcorr(rx_data_seq,sync_time);
            figure('Name','時間同步:確認是否正確同步','NumberTitle','off');
            plot(lags,abs(temp));grid on;
            ...
            for i = 1:size(Inx_RS,2)-1
                CE_freq(Inx_carrier,Inx_RS(1,i)) = rx_data_aft_freq(Inx_carrier,Inx_RS(1,i))...
                                                    ./tx_data_freq(Inx_carrier,Inx_RS(1,i));
                CE_freq_RS(Inx_carrier,i) = rx_data_aft_seq_RS_freq(Inx_carrier,i)...
                                                    ./tx_data_freq(Inx_carrier,Inx_RS(1,i+1));
            ...
            plot(f,abs(fftshift(CE_freq(:,Inx_RS))));
            xlabel("Freq.")
            ylabel("Amplitude")
        ...
        rx_radio_HS=set_pluto_RX_HS(fc_HS,fs,feedback_signal,Rx_gain);
        while true
            [rx_feedback, valid, overflow] =  rx_radio_HS();
    
            [temp, lags] = xcorr(rx_feedback,feedback_signal); %交相關
    ...
    %         stem(lags(TF2),abs(a(TF2))); %符合時間點圖
        ...
        %釋放tx rx接口
        release(rx_radio_HS);release(tx_radio_HS);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%   BER   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
